# Script mejorado para renombrar archivos que contengan nombres en español
...
function Update-References {
    ...
    foreach ($dir in $searchDirectories) {
        ...
        try {
            $files = Get-ChildItem -Path $dir -Recurse -Include "*.ts", "*.js", "*.json", "*.yml", "*.yaml" | Where-Object { Should-ProcessFile $_.FullName }
...
function Rename-Files {
    ...
    # Directorios específicos para buscar archivos
    $searchDirectories = @(
        "calculadora-electrica-backend/src",
        "calculadora-electrica-backend/test",
        "calculadora-electrica-backend/scripts",
        "calculadora-electrica-backend/database",
        "calculadora-electrica-frontend/src"
    )
    ...
    foreach ($oldName in $fileRenames.Keys) {
        ...
        foreach ($dir in $searchDirectories) {
            ...
            try {
                ...
                $files = Get-ChildItem -Path $dir -Recurse -Filter $oldName | Where-Object { Should-ProcessFile $_.FullName }
...
function Revert-Renames {
    ...
    foreach ($oldName in $fileRenames.Keys) {
        ...
        $files = Get-ChildItem -Path "." -Recurse -Filter $newName | Where-Object { Should-ProcessFile $_.FullName }
