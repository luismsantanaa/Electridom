...
export class LoggingInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    // Log del request
    this.logger.logRequest(requestId, method, url, {
      userAgent,
      ip,
    ...
    return next.handle().pipe(
      tap({
        next: (data) => {
          ...
          // Log del response
          this.logger.logResponse(requestId, method, url, statusCode, duration, {
        ...
        error: (error) => {
          ...
          // Log del error
          this.logger.logError(requestId, method, url, error, {
