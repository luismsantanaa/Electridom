...
export async function seedResistivity(dataSource: DataSource): Promise<void> {
  ...
  if (existingCount > 0) {
    console.log('✅ Datos de resistivity ya existen, saltando seed...');
    return;
  }
  ...
  const entities = resistividades.map((resistivity) => {
    const entity = new Resistivity();
    entity.material = resistivity.material;
    entity.seccionMm2 = resistivity.seccionMm2;
    entity.ohmKm = resistivity.ohmKm;
    entity.notes = resistivity.notes;
